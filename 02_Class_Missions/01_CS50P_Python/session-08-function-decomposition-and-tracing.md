...
|---|---|---|
| 0–8 min | Skill Warm-Up | Trace one function call and return path using a supplied code fragment. |
...
Complete the assigned segments before class or in a separately scheduled resource session. The full package is longer than the eight-minute Skill Warm-Up.

## 1. Skill Warm-Up

Students inspect a short program with `main`, a helper function, and a return value, then complete:
...
Local variable:
Returned value:
Final use of the returned value:
```
...
- rebuilt two-function program;
- one explanation of a common return-value mistake.
