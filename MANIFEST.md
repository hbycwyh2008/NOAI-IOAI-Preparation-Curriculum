...
| Layer | Current state |
|---|---|
| Machine-readable source of truth | `curriculum_spec.json` |
...
| Executable pathway layer | exact NOAI Round 1, NOAI Round 2, and IOAI full-extension documents |
| Daily model-recognition layer | 36 public scenarios, reusable answer record, private-key protocol, mastery rule |
...
1. `curriculum_spec.json` — counts, Phase ranges, required execution documents, and evidence boundaries;
...
- `scripts/validate_curriculum_spec.py` verifies that the machine-readable specification matches launchers, packet counts, executable routes, drill-bank minimums, and evidence boundaries.
